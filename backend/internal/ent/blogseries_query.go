...
type BlogSeriesQuery struct {
	...
	withTranslations *BlogSeriesTranslationQuery
...
func (bsq *BlogSeriesQuery) QueryTranslations() *BlogSeriesTranslationQuery {
	query := (&BlogSeriesTranslationClient{config: bsq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := bsq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (bsq *BlogSeriesQuery) Clone() *BlogSeriesQuery {
	...
	return &BlogSeriesQuery{
		...
		withTranslations: bsq.withTranslations.Clone(),
...
func (bsq *BlogSeriesQuery) WithTranslations(opts ...func(*BlogSeriesTranslationQuery)) *BlogSeriesQuery {
	query := (&BlogSeriesTranslationClient{config: bsq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	bsq.withTranslations = query
	return bsq
}
...
func (bsq *BlogSeriesQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*BlogSeries, error) {
	...
	if query := bsq.withTranslations; query != nil {
		if err := bsq.loadTranslations(ctx, query, nodes,
			func(n *BlogSeries) { n.Edges.Translations = []*BlogSeriesTranslation{} },
			func(n *BlogSeries, e *BlogSeriesTranslation) { n.Edges.Translations = append(n.Edges.Translations, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (bsq *BlogSeriesQuery) loadBlogPosts(ctx context.Context, query *BlogPostQuery, nodes []*BlogSeries, init func(*BlogSeries), assign func(*BlogSeries, *BlogPost)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(blogpost.FieldSeriesID)
	}
...
func (bsq *BlogSeriesQuery) loadTranslations(ctx context.Context, query *BlogSeriesTranslationQuery, nodes []*BlogSeries, init func(*BlogSeries), assign func(*BlogSeries, *BlogSeriesTranslation)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(blogseriestranslation.FieldBlogSeriesID)
	}
	query.Where(predicate.BlogSeriesTranslation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(blogseries.TranslationsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "blog_series_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil

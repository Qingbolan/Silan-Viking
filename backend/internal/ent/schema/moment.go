...
// Moment holds the schema definition for the Moment entity.
type Moment struct {
	ent.Schema
}

// Annotations for the Moment schema.
func (Moment) Annotations() []schema.Annotation {
	return []schema.Annotation{
		entsql.Annotation{Table: "moments"},
	}
}
...
func (Moment) Fields() []ent.Field {
	return []ent.Field{
		...
		// M0.5a §11.7.1: moments is promoted to the content main table
		// of the `moment` type (ruling #3). It needs a stable slug.
		field.String("slug").
			MaxLen(200).
			Unique().
			NotEmpty(),
		// Renamed from `type` (M0.5a §11.7.1): "which kind of thing this
		// update is about" — kept alongside moment_type, not the same axis.
		field.Enum("subject_kind").
			Values("work", "education", "research", "publication", "project").
			Default("project").
			StorageKey("type"),
		// M0.5a §11.7.1 / 10 §10.4.6: the 8 moment_type values — "what kind
		// of update this is".
		field.Enum("moment_type").
...
func (Moment) Indexes() []ent.Index {
	return []ent.Index{
		index.Fields("moment_type"),
...
func (Moment) Edges() []ent.Edge {
	return []ent.Edge{
		edge.To("translations", MomentTranslation.Type),
	}
}

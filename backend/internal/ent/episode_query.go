...
type EpisodeQuery struct {
	...
	withSeries       *EpisodeSeriesQuery
	withTranslations *EpisodeTranslationQuery
...
func (eq *EpisodeQuery) QuerySeries() *EpisodeSeriesQuery {
	query := (&EpisodeSeriesClient{config: eq.config}).Query()
...
func (eq *EpisodeQuery) QueryTranslations() *EpisodeTranslationQuery {
	query := (&EpisodeTranslationClient{config: eq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (eq *EpisodeQuery) WithSeries(opts ...func(*EpisodeSeriesQuery)) *EpisodeQuery {
	query := (&EpisodeSeriesClient{config: eq.config}).Query()
...
func (eq *EpisodeQuery) WithTranslations(opts ...func(*EpisodeTranslationQuery)) *EpisodeQuery {
	query := (&EpisodeTranslationClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (eq *EpisodeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Episode, error) {
	...
	if query := eq.withSeries; query != nil {
		if err := eq.loadSeries(ctx, query, nodes, nil,
			func(n *Episode, e *EpisodeSeries) { n.Edges.Series = e }); err != nil {
			return nil, err
		}
	}
	if query := eq.withTranslations; query != nil {
		if err := eq.loadTranslations(ctx, query, nodes,
			func(n *Episode) { n.Edges.Translations = []*EpisodeTranslation{} },
			func(n *Episode, e *EpisodeTranslation) { n.Edges.Translations = append(n.Edges.Translations, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (eq *EpisodeQuery) loadTranslations(ctx context.Context, query *EpisodeTranslationQuery, nodes []*Episode, init func(*Episode), assign func(*Episode, *EpisodeTranslation)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(episodetranslation.FieldEpisodeID)
	}
	query.Where(predicate.EpisodeTranslation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(episode.TranslationsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err

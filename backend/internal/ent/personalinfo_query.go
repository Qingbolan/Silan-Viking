...
func (piq *PersonalInfoQuery) QueryTranslations() *PersonalInfoTranslationQuery {
	query := (&PersonalInfoTranslationClient{config: piq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := piq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QuerySocialLinks chains the current query on the "social_links" edge.
func (piq *PersonalInfoQuery) QuerySocialLinks() *SocialLinkQuery {
	query := (&SocialLinkClient{config: piq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := piq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (piq *PersonalInfoQuery) WithTranslations(opts ...func(*PersonalInfoTranslationQuery)) *PersonalInfoQuery {
	query := (&PersonalInfoTranslationClient{config: piq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	piq.withTranslations = query
	return piq
}

// WithSocialLinks tells the query-builder to eager-load the nodes that are connected to
// the "social_links" edge. The optional arguments are used to configure the query builder of the edge.
func (piq *PersonalInfoQuery) WithSocialLinks(opts ...func(*SocialLinkQuery)) *PersonalInfoQuery {
	query := (&SocialLinkClient{config: piq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	piq.withSocialLinks = query
	return piq
}
...
func (piq *PersonalInfoQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*PersonalInfo, error) {
	...
	if query := piq.withTranslations; query != nil {
		if err := piq.loadTranslations(ctx, query, nodes,
			...
			func(n *PersonalInfo, e *PersonalInfoTranslation) {
				n.Edges.Translations = append(n.Edges.Translations, e)
			}); err != nil {
			return nil, err
		}
	}
	if query := piq.withSocialLinks; query != nil {
		if err := piq.loadSocialLinks(ctx, query, nodes,
			func(n *PersonalInfo) { n.Edges.SocialLinks = []*SocialLink{} },
			func(n *PersonalInfo, e *SocialLink) { n.Edges.SocialLinks = append(n.Edges.SocialLinks, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (piq *PersonalInfoQuery) loadTranslations(ctx context.Context, query *PersonalInfoTranslationQuery, nodes []*PersonalInfo, init func(*PersonalInfo), assign func(*PersonalInfo, *PersonalInfoTranslation)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(personalinfotranslation.FieldPersonalInfoID)
	}
	query.Where(predicate.PersonalInfoTranslation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(personalinfo.TranslationsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "personal_info_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (piq *PersonalInfoQuery) loadSocialLinks(ctx context.Context, query *SocialLinkQuery, nodes []*PersonalInfo, init func(*PersonalInfo), assign func(*PersonalInfo, *SocialLink)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(sociallink.FieldPersonalInfoID)
	}
	query.Where(predicate.SocialLink(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(personalinfo.SocialLinksColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err

...
type ProjectTechnologyQuery struct {
	config
	ctx         *QueryContext
	order       []projecttechnology.OrderOption
	inters      []Interceptor
...
func (ptq *ProjectTechnologyQuery) Order(o ...projecttechnology.OrderOption) *ProjectTechnologyQuery {
...
func (ptq *ProjectTechnologyQuery) All(ctx context.Context) ([]*ProjectTechnology, error) {
	...
	qr := querierAll[[]*ProjectTechnology, *ProjectTechnologyQuery]()
	return withInterceptors[[]*ProjectTechnology](ctx, ptq, qr, ptq.inters)
}
...
func (ptq *ProjectTechnologyQuery) Count(ctx context.Context) (int, error) {
	...
	return withInterceptors[int](ctx, ptq, querierCount[*ProjectTechnologyQuery](), ptq.inters)
}
...
func (ptq *ProjectTechnologyQuery) Clone() *ProjectTechnologyQuery {
	...
	return &ProjectTechnologyQuery{
		config:      ptq.config,
		ctx:         ptq.ctx.Clone(),
		order:       append([]projecttechnology.OrderOption{}, ptq.order...),
...
func (ptq *ProjectTechnologyQuery) GroupBy(field string, fields ...string) *ProjectTechnologyGroupBy {
	...
	grbuild := &ProjectTechnologyGroupBy{build: ptq}
...
func (ptq *ProjectTechnologyQuery) Select(fields ...string) *ProjectTechnologySelect {
	...
	sbuild := &ProjectTechnologySelect{ProjectTechnologyQuery: ptq}
	sbuild.label = projecttechnology.Label
	sbuild.flds, sbuild.scan = &ptq.ctx.Fields, sbuild.Scan
	return sbuild
}

// Aggregate returns a ProjectTechnologySelect configured with the given aggregations.
func (ptq *ProjectTechnologyQuery) Aggregate(fns ...AggregateFunc) *ProjectTechnologySelect {
...
func (ptq *ProjectTechnologyQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := ptq.ctx.Fields; len(fields) > 0 {
		...
		if ptq.withProject != nil {
			_spec.Node.AddColumnOnce(projecttechnology.FieldProjectID)
		}
	}
...
func (ptgb *ProjectTechnologyGroupBy) Scan(ctx context.Context, v any) error {
	ctx = setContextOp(ctx, ptgb.build.ctx, ent.OpQueryGroupBy)
	if err := ptgb.build.prepareQuery(ctx); err != nil {
		return err
	}
	return scanWithInterceptors[*ProjectTechnologyQuery, *ProjectTechnologyGroupBy](ctx, ptgb.build, ptgb, ptgb.build.inters, v)
}

func (ptgb *ProjectTechnologyGroupBy) sqlScan(ctx context.Context, root *ProjectTechnologyQuery, v any) error {
...
func (pts *ProjectTechnologySelect) Aggregate(fns ...AggregateFunc) *ProjectTechnologySelect {
	pts.fns = append(pts.fns, fns...)
	return pts
}

// Scan applies the selector query and scans the result into the given value.
func (pts *ProjectTechnologySelect) Scan(ctx context.Context, v any) error {
	ctx = setContextOp(ctx, pts.ctx, ent.OpQuerySelect)
	if err := pts.prepareQuery(ctx); err != nil {
		return err
	}
	return scanWithInterceptors[*ProjectTechnologyQuery, *ProjectTechnologySelect](ctx, pts.ProjectTechnologyQuery, pts, pts.inters, v)
}

func (pts *ProjectTechnologySelect) sqlScan(ctx context.Context, root *ProjectTechnologyQuery, v any) error {

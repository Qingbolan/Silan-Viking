...
type ProjectDetailQuery struct {
	...
	withTranslations *ProjectDetailTranslationQuery
...
func (pdq *ProjectDetailQuery) QueryTranslations() *ProjectDetailTranslationQuery {
	query := (&ProjectDetailTranslationClient{config: pdq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pdq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pdq *ProjectDetailQuery) FirstID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (pdq *ProjectDetailQuery) FirstIDX(ctx context.Context) string {
...
func (pdq *ProjectDetailQuery) OnlyID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (pdq *ProjectDetailQuery) OnlyIDX(ctx context.Context) string {
...
func (pdq *ProjectDetailQuery) IDs(ctx context.Context) (ids []string, err error) {
...
func (pdq *ProjectDetailQuery) IDsX(ctx context.Context) []string {
	ids, err := pdq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (pdq *ProjectDetailQuery) Clone() *ProjectDetailQuery {
	...
	return &ProjectDetailQuery{
		...
		withTranslations: pdq.withTranslations.Clone(),
...
func (pdq *ProjectDetailQuery) WithTranslations(opts ...func(*ProjectDetailTranslationQuery)) *ProjectDetailQuery {
	query := (&ProjectDetailTranslationClient{config: pdq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pdq.withTranslations = query
	return pdq
}
...
func (pdq *ProjectDetailQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*ProjectDetail, error) {
	...
	if query := pdq.withTranslations; query != nil {
		if err := pdq.loadTranslations(ctx, query, nodes,
			func(n *ProjectDetail) { n.Edges.Translations = []*ProjectDetailTranslation{} },
			func(n *ProjectDetail, e *ProjectDetailTranslation) {
...
func (pdq *ProjectDetailQuery) loadProject(ctx context.Context, query *ProjectQuery, nodes []*ProjectDetail, init func(*ProjectDetail), assign func(*ProjectDetail, *Project)) error {
	...
	nodeids := make(map[string][]*ProjectDetail)
	for i := range nodes {
...
func (pdq *ProjectDetailQuery) loadTranslations(ctx context.Context, query *ProjectDetailTranslationQuery, nodes []*ProjectDetail, init func(*ProjectDetail), assign func(*ProjectDetail, *ProjectDetailTranslation)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(projectdetailtranslation.FieldProjectDetailID)
	}
	query.Where(predicate.ProjectDetailTranslation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(projectdetail.TranslationsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.ProjectDetailID
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "project_detail_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (pdq *ProjectDetailQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(projectdetail.Table, projectdetail.Columns, sqlgraph.NewFieldSpec(projectdetail.FieldID, field.TypeString))
	_spec.From = pdq.sql
	...
	if fields := pdq.ctx.Fields; len(fields) > 0 {
		...
		if pdq.withProject != nil {
			_spec.Node.AddColumnOnce(projectdetail.FieldProjectID)
		}
	}

//! `MomentParser` — the `Parser` for the `moment` content type.
//!
//! `moment` is the 6th content type (`docs/silan-viking/10` §10.4.6 ruling
...
/// The parser for `moment` Items.
pub struct MomentParser {
    schema: Arc<Schema>,
}
...
impl Parser for MomentParser {
    fn content_type(&self) -> ContentKind {
        ContentKind::Moment
    }

    fn parse(&self, item: &Item) -> Result<Parsed, ParseError> {
        ProseTypeParser::new(&self.schema).parse(ContentKind::Moment, item)
    }

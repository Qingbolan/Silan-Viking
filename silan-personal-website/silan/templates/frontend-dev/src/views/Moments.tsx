                    ...
                    >
                      ...
                      <article className="min-w-0">
                        ...
                        {moment.related_outputs?.length > 0 && (
                          <div className="mt-6 max-w-[68ch] border-y border-ds-border py-3">
                            <div className="mb-2 font-mono text-ds-xs font-semibold uppercase tracking-[0.12em] text-ds-fg-subtle">
                              {copy.outputs}
                            </div>
                            <div className="divide-y divide-ds-border">
                              {moment.related_outputs.map((output) => (
                                <Link
                                  key={`${output.kind}-${output.id}`}
                                  to={output.path}
                                  className="group grid grid-cols-[minmax(0,1fr)_auto] gap-3 py-3 outline-none focus-visible:rounded-ds-sm focus-visible:shadow-ds-focus"
                                >
                                  <span className="min-w-0">
                                    <span className="mb-1 inline-flex items-center gap-1.5 text-ds-xs font-medium text-ds-primary">
                                      {copy.outputKinds[output.kind]}
                                      {output.relation && (
                                        <span className="font-normal text-ds-fg-subtle">/{output.relation}</span>
                                      )}
                                    </span>
                                    <span className="block truncate text-ds-sm font-medium text-ds-fg group-hover:text-ds-primary">
                                      {output.title}
                                    </span>
                                    {output.description && (
                                      <span className="mt-0.5 line-clamp-2 block text-ds-xs leading-5 text-ds-fg-muted">
                                        {output.description}
                                      </span>
                                    )}
                                  </span>
                                  <ArrowUpRight className="mt-1 size-3.5 text-ds-fg-subtle transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5 group-hover:text-ds-primary" aria-hidden />
                                </Link>
                              ))}
                            </div>
                          </div>

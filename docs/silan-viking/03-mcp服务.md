...
> 一面三工具一面四命令的错位。它们走 **sync-then-query 模型**(owner 裁定的
> 设计变更):运行时数据(评论/打点)只在生产服务器(`01` §1.8),`silan
> stats sync` 经 Go API 把数据拉进本地 `portfolio.db` 的 `stats_cache_*`
> 表,这四个 MCP 工具与 CLI 读命令**读同一份本地缓存**(`02` §`silan stats`)。
> CLI 给 silan、MCP 给协作 agent,同一份缓存,不存在两套逻辑。它们是
> **只读**:agent/owner 能看交互数据,不能改 —— 打点由 Go API 在访客访问
> 时写入服务器 db。缓存未 `sync` 过时,工具返回 `backend_unavailable`。
...
- 第 4 步推进指针前,若主分支 HEAD 已被推进(同一台机器上 `ctx_write` /
  `reflect` 与本次 `accept` 交错),临时区那个 merge 不再基于最新主分支 →
  accept 报错,silan 重跑(或先 `rebase`)。`expected_head` 防的是**单机内**
  写操作交错,绝不丢 commit。**跨设备不在此机制保证范围内** —— silan-viking
  是单设备假设,多设备靠 `content/` 仓手动 `git push/pull`(见 `17` §17.3)。
...
错误码:`silan-viking-mcp` 的 `McpError` 是一个**带 `String` 载荷的语义
enum**(实现选择 —— 早期设计是「闭集 code 字符串」,实现时改成 Rust enum
变体,每个变体携带 human-readable detail)。变体集:

| 变体 | 触发 |
|---|---|
| `InvalidRequest` | 参数类型/必填字段不对,或 `type` 不在 6 个 content type 闭集 |
| `InvalidAgentUri` | `ctx_read`/`ctx_write` 的 URI 不以 `silan://agent/` 开头(含 `..` 穿越) |
| `Workspace` | Item / Part / URI 不存在,或 workspace 操作失败 |
| `Proposal` | `capture`/`propose` 失败,提案未过 SCHEMA 校验,或 Git 冲突 |
| `BackendUnavailable` | stats 本地缓存未 `sync`,或本地 Git / SQLite 不可用 |
| `Io` | 文件读写错误 |
| `UnknownTool` | `tools/call` 的工具名不在已通告的 18 工具闭集内(见下)|

> **18 工具闭集**(`UnknownTool` 的判定依据,也是 `tools/list` 通告的全集):
> 档1 只读 10 个 —— `recall` `list` `browse` `read` `context_brief` `lint`
> `stats` `visitors` `crawler_breakdown` `source_breakdown`;
> 档2 捕捉 1 个 —— `capture`;
> 档2.5 agent context 4 个 —— `ctx_read` `ctx_write` `ctx_brief` `reflect`;
> 档3 提案 2 个 —— `propose` `summarize_updates`;
> 档4 危险 1 个 —— `deploy`(默认不通告,`--enable-deploy` 才进闭集)。
> 注意 `context_brief`(档1,读发布内容现状)与 `ctx_brief`(档2.5,读
> `agent/` 记忆)是**两个不同工具**,勿合并。E 阶段的 `suggest_*` /
> `propose_schema` 不在此 18 内 —— 见 `15` §15.5 与 `04` 的 E1/E2 里程碑。

> 通用返回形态仍是 `{ "error": { ... } }`;`code` 字段取变体名的
> snake_case 形式(`invalid_request` / `invalid_agent_uri` / …),`message`
> 是变体携带的 detail。这是实现与本节早期「6 值闭集」的一处分歧:实现的
> 变体更细(把 `permission_denied` 拆成 `InvalidAgentUri`、把 `not_found`
> 并入 `Workspace`),本表已对齐实现。

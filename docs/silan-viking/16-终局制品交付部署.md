...
- `remote_dir` 若指向 `deploy` 用户无写权限的路径(如 `/srv` 下),
  `mkdir` 报裸 `Permission denied`。应给清晰提示,说明需管理员预先
  `chown` 部署目录,或把 `remote_dir` 选在用户家目录下。
- 跨机 ship 前应清空 `remote_dir` 的旧文件:一旦某次部署中途失败、
  Docker 把某个 bind-mount 源创建成同名目录,后续部署会被这个残留
  目录持续卡住(`rm -f` 删不掉目录)。
- 跨平台容器构建(macOS 主机 + Linux 容器共享挂载卷)不能信 cargo
  增量缓存,需独立 `CARGO_TARGET_DIR` 并在改动后干净重建。

...
```
silan stats show <uri>        某 Item / 章节(--section)的浏览/点赞/评论统计
...
> **`stats` 是远程查询,不查本地 db**。运行时数据(评论/打点)只在生产
> 服务器(`01` §1.8)—— 本地 `portfolio.db` 的 `content_interaction` 表
> **是空的**。`silan stats` 通过 HTTP 调**服务器上 Go API 的 stats
> endpoint**(`/api/v1/stats/...`),服务器地址取 `silan-viking.toml`
> 的 `[deploy].host`。查的是服务器实时数据,不是本地表。
> 未部署(无 `[deploy]`)时 `silan stats` 报错提示「运行时数据需先部署」。
...
```
silan mcp serve     启动 MCP server,供协作 agent 接入(握手推 SCHEMA,见 06 §6.3)
silan mcp status    查看接入情况
```
